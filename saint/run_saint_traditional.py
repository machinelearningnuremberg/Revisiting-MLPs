...
def objective(trial, X_outer, Y_outer, batch_size, eval_batch_size, start_time,
              time_limit):
    # Search space
    ...
    best_rocauc = 0.0
    train_idx, test_idx = train_test_split(range(len(X_outer)), test_size=0.5, random_state=args.seed,
                                           stratify=Y_outer)
    X = D.build_X(
