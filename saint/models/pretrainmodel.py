...
class SAINT(nn.Module):
    ...
    def forward(self, x_categ, x_cont):
        if x_categ is None:
            # Handle the case when only continuous data is provided
            if self.cont_embeddings == 'MLP':
                x_cont = torch.stack([self.simple_MLP[i](x_cont[:, i].view(-1, 1)) for i in range(self.num_continuous)],
                                     dim=1)
            # Process continuous data
            x_cont = self.norm(x_cont)
            x = x_cont
        elif x_cont is None:
            # Handle the case when only categorical data is provided
            x_categ = torch.where(x_categ == self.unknown_value, self.categories.to(x_categ.device), x_categ)
            x = self.embeds(x_categ + self.categories_offset)
        else:
            # Handle the case when both categorical and continuous data is provided
            x_categ = torch.where(x_categ == self.unknown_value, self.categories.to(x_categ.device), x_categ)
            x_categ = self.embeds(x_categ + self.categories_offset)
            if self.cont_embeddings == 'MLP':
                x_cont = torch.stack([self.simple_MLP[i](x_cont[:, i].view(-1, 1)) for i in range(self.num_continuous)],
                                     dim=1)
            x_cont = self.norm(x_cont)
        ...
        cat_outs = self.mlp1(x[:, :self.num_categories, :]) if x_categ is not None else None
        con_outs = self.mlp2(x[:, self.num_categories:, :]) if x_cont is not None else None
