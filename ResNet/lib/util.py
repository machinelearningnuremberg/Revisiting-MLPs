...
def get_categories(
    X_cat: ty.Optional[torch.Tensor], train_idx: ty.List, test_idx: ty.List
    ...
    if X_cat is None:
        ...
        for i in range(X_cat.shape[1]):
            # Combine unique categories from both training and testing indices for each feature
            unique_categories = set(X_cat[train_idx][:, i].tolist()) | set(X_cat[test_idx][:, i].tolist())
