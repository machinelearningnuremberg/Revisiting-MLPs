            ...
            for init_feat_idx in range(input_dim):
                if self.continuous_idx[init_feat_idx] == 1:
                    ...
                    self.embedding_group_matrix[group_idx, post_emb_idx] = group_matrix[
                        group_idx, init_feat_idx]  # noqa
                    post_emb_idx += 1
                else:
                    # this is a categorical feature which creates multiple embeddings
                    n_embeddings = cat_emb_dims[cat_feat_counter]
                    self.embedding_group_matrix[group_idx, post_emb_idx:post_emb_idx + n_embeddings] = group_matrix[
                                                                                                           group_idx, init_feat_idx] / n_embeddings  # noqa
    ...
    def forward(self, x):
        ...
        for feat_init_idx, is_continuous in enumerate(self.continuous_idx):
            ...
            if is_continuous:
                ...
                x[:, feat_init_idx] = torch.where(x[:, feat_init_idx] >
                                                  self.embeddings[cat_feat_counter].num_embeddings - 1,
                                                  torch.ones_like(x[:, feat_init_idx]) * self.embeddings[
                                                  cat_feat_counter].num_embeddings - 1, x[:, feat_init_idx])
