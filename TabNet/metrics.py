...
class AUC(Metric):
    ...
    def __call__(self, y_true, y_score):
        ...
        """
        nr_classes = len(np.unique(y_true))
        # Use 'raise' for binary classification, 'ovo' for multi-class
        strategy = 'raise' if nr_classes == 2 else 'ovo'
        y_score = y_score if nr_classes > 2 else y_score[:, 1]

        return roc_auc_score(y_true, y_score, multi_class=strategy)
