...
parser.add_argument('--outer_fold', type=int, default=1, help='The outer fold to use. Default is 0')
...
def load_best_config(project_name, dataset_name, outer_fold, num_trials=30):
    ...
    hp_keys = ['n_layers', 'd_token', 'residual_dropout', 'attn_dropout', 'ffn_dropout', 'd_ffn_factor', 'lr',
               'weight_decay']
    best_config = None
...
def run_single_outer_fold(outer_fold, D, outer_folds):
    ...
    best_params = load_best_config('t4tab/FT_optuna_original', D.info['dataset_name'], args.outer_fold)
